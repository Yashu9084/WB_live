    ...
    async function loginHandler() {
        try {
            ...
            let response = await fetch(url, {
                ...
                body: JSON.stringify({ phone: loginCredential.phone, otp_code: loginCredential.otp_code })
            })
            response = await response.json()
            if (response.success) { 
                ...
                setCookie(null, '@VendorApp', JSON.stringify({ "token": `${response.token}` }), {
                    maxAge: 86400,
                    path: '/',
                })
                ...
                setIsSent(response.success === true)
                setIsLoading(false);
                localStorage.setItem('@VendorApp', JSON.stringify({ "token": `${response.token}` }))
                ...
                <div className="login-form">
                    <div className="card">
                        ...
                        <div className="form-item">
                            <input type="number" placeholder="+91 " name="phone" value={loginCredential.phone} onChange={(e) => formInputHandler(e)} />
                            <AiFillPhone className="placeholder-icon" />
                        </div>
                        {
                            isSent ? (
                                <div className="form-item">
                                    <input type="number" placeholder="OTP " name="otp_code" value={loginCredential.otp_code} onChange={(e) => formInputHandler(e)} disabled={!isSent} />
                        ...
                        <ButtonDark onClick={isSent ? loginHandler : sendOTPHandler}>{isLoading ? <Spinner1 /> : isSent ? "Login" : "Send OTP"}</ButtonDark>
                        {/* <div className="already-have-account">
